...
func TestCatalogDocMDPReference(t *testing.T) {
	...
	for _, certType := range []sign.CertType{sign.CertificationSignature, sign.ApprovalSignature} {
		t.Run(certType.String(), func(t *testing.T) {
			...
			if err := sign.SignFile("../testfiles/testfile20.pdf", outputFile.Name(), sign.SignData{
				Signature:   sign.SignDataSignature{CertType: certType},
				Signer:      pkey,
				Certificate: cert,
			}); err != nil {
				t.Fatal(err)
			...
			if certType == sign.CertificationSignature {
